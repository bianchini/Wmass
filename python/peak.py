...
class Peak:
    ...
    def initialize_central(self, deg=2, addbins=2):
        if self.charges=='toy':
            ...
            h_plus  = self.f.Get('WpreFSR_mu_'+self.var[0]+'_'+self.charges+'_'+self.cut+'_w0').Clone('h_plus')
            if len(self.var)>1:
                h_plus.Add(  self.f.Get('WpreFSR_mu_'+self.var[1]+'_'+self.charges+'_'+self.cut+'_w0') )
        else:
            h_plus  = self.f.Get('WpreFSR_mu_'+self.var[0]+'_plus_'+self.cut+'_w0').Clone('h_plus')
            h_minus = self.f.Get('WpreFSR_mu_'+self.var[0]+'_minus_'+self.cut+'_w0').Clone('h_minus')
            h_plus.Add(h_minus)    
            if len(self.var)>1:
                h_plus.Add(  self.f.Get('WpreFSR_mu_'+self.var[1]+'_plus_'+self.cut+'_w0') )
                h_plus.Add(  self.f.Get('WpreFSR_mu_'+self.var[1]+'_minus_'+self.cut+'_w0') )
    ...
    def get_peak(self, deg=2, addbins=2, der=0, weight=0, ntoys=-1, do_plot=True, do_mean=False):        
        ...
        if self.charges=='toy':
            ...
            h_plus  = self.f.Get('WpreFSR_mu_'+self.var[0]+'_'+self.charges+'_'+self.cut+'_w'+str(weight)).Clone('h_plus')
            if len(self.var)>1:
                h_plus.Add(  self.f.Get('WpreFSR_mu_'+self.var[1]+'_'+self.charges+'_'+self.cut+'_w'+str(weight)))            
        else:
            h_plus = self.f.Get('WpreFSR_mu_'+self.var[0]+'_plus_'+self.cut+'_w'+str(weight)).Clone('h_plus')
            h_minus = self.f.Get('WpreFSR_mu_'+self.var[0]+'_minus_'+self.cut+'_w'+str(weight)).Clone('h_minus')
            h_plus.Add(h_minus)
            if len(self.var)>1:
                h_plus.Add(  self.f.Get('WpreFSR_mu_'+self.var[1]+'_plus_'+self.cut+'_w'+str(weight)) )
                h_plus.Add(  self.f.Get('WpreFSR_mu_'+self.var[1]+'_minus_'+self.cut+'_w'+str(weight)) )
        ...
        if self.charges=='both':
            charge_label = '$W^\pm$'
        if self.tag=='_acc':
            charge_label += ', within acceptance'
        else:
            charge_label += ', full phase-space'
    ...
    def run_fits(self, degs=[4], addbins=[5], do_systematics=False, weights_scale=[0], weights_pdf=[9]):
        for deg in degs:
            ...
            for iaddbin,addbin in enumerate(addbins):
                ...
                for ider,der in enumerate(range(0, deg)):
                    ...
                    for iw,w in enumerate(weights_mass):
                        extremals_mass[iw] = self.get_peak(deg=deg, addbins=addbin, der=der, weight=iw, ntoys=-1, do_plot=False, do_mean=do_mean)
                    ...
                    ax.errorbar(x=np.array(weights_mass)+ider*0.002, y=(extremals_mass+1)*self.M, xerr=0, yerr=extremals_err_mass*self.M, fmt=fmts[ider], color=colors[ider], linewidth=1,
                                label=(('$\hat{x}_{'+str(der)+'}$=' if der>0 else '$x_{\mu}$=')+'{:0.4f}'.format(lsq[0])+'$\cdot M_{W}$ '+('+' if lsq[1]>0 else '')+'{:0.3f}'.format(lsq[1])+' GeV')
                                )
                plt.grid(True)
                ax.plot(weights_mass, weights_mass, 'k--', label=r'Ideal', linewidth=3.0)
                legend = ax.legend(loc='best', shadow=False, fontsize='x-large')
                plt.axis([80.100, 80.750, 78.800, 80.800])
                plt.show()
                charge_label = '$W^+$' if self.charges=='plus' else '$W^-$'
                if self.charges=='both':
                    charge_label = '$W^\pm$'                    
    ...
    def run_mean_vs_range(self, degs=[4], addbins=[5], do_systematics=False, weights_scale=[0], weights_pdf=[9]):
