...
class FixPoint:
    def __init__(self, x_low=0.92, x_high=1.08, nbins=201, p0=0.5, p1=1.0, p2=0.0, pR=3.0, pdf_type='power'):
        print 'Initialize'
    ...
    def pdf_gamma(self, gamma):
        if self.pdf_type=='exp':
            ...
            pdf = math.pow(gamma-self.p0,self.p1) + self.p2 if (gamma<self.pR and gamma>=1.000) else 0.0 
        return pdf
...
fixpoint = FixPoint(x_low=0.5, x_high=1.5, nbins=101, p0=float(argv[3]), p1=float(argv[4]), p2=float(argv[5]), pR=float(argv[6]), pdf_type='power')
...
if len(argv)>7:    
    if fixpoint.pdf_type=='exp':
        leg.SetHeader('#splitline{g(#gamma)=(#gamma-'+'{:0.1f}'.format(fixpoint.p0)+')exp(-'+'{:0.1f}'.format(fixpoint.p1)+'(#gamma-'+'{:0.1f}'.format(fixpoint.p0)+'))'+', #gamma#leq'+'{:0.1f}'.format(fixpoint.pR)+'}{A_{0}='+('{:0.1f}'.format(float(argv[7])) if float(argv[7])!=0.6667 else '2/3')+', A_{4}='+'{:0.1f}'.format(float(argv[8]))+'}')
    ...
    if fixpoint.pdf_type=='exp':
        leg.SetHeader('#splitline{g(#gamma)=(#gamma-'+'{:0.1f}'.format(fixpoint.p0)+')exp(-'+'{:0.1f}'.format(fixpoint.p1)+'(#gamma-'+'{:0.1f}'.format(fixpoint.p0)+'))'+', #gamma#leq'+'{:0.1f}'.format(fixpoint.pR)+'}{A_{0}=tanh(4(#gamma-1)), A_{4}=tanh(0.3(#gamma-1))}')
    elif fixpoint.pdf_type=='power':
        leg.SetHeader('#splitline{g(#gamma)=(#gamma-'+'{:0.1f}'.format(fixpoint.p0)+')^{'+'{:0.1f}'.format(fixpoint.p1)+'}, #gamma#leq'+'{:0.1f}'.format(fixpoint.pR)+'}{A_{0}=tanh(4(#gamma-1)), A_{4}=tanh(0.3(#gamma-1))}')
